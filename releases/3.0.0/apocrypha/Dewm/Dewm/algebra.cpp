...
Point3D Point2D::make3D()
{
  return Point3D(v_[0], v_[1], 0);
}

void Vector3D::draw(Point3D origin)
{
  glPointSize(5);
  glBegin(GL_POINTS);
  ...
  Point3D destination = origin + *this;
  glBegin(GL_LINES);
  {
    glVertex3d(origin[0], origin[1], origin[2]);  
    glVertex3d(destination[0], destination[1], destination[2]);  
  }
  glEnd();
}
