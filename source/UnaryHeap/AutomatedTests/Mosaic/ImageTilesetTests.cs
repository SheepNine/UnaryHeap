...
{
    [TestFixture]
    public class ImageTilesetTests
    {
        [Test]
        public void Constructor()
        {
            ...
            using (var sut = new ImageTileset(image, 10))
            {
        ...
        public void DrawTile()
        {
            ...
            using (var sut = new ImageTileset(tiles, 8))
            {
        ...
        public void SimpleArgumentExceptions()
        {
            ...
            using (var g = Graphics.FromImage(imgo))
            {
                ...
                Assert.Throws<ArgumentNullException>(
                    () => { new ImageTileset(null, 2); });
                Assert.Throws<ArgumentOutOfRangeException>(
                    () => { new ImageTileset(img, 0); });
                Assert.Throws<ArgumentException>(
                    () => { new ImageTileset(img, 3); });
                Assert.Throws<ArgumentException>(
                    () => { new ImageTileset(img, 4); });

                var sut = new ImageTileset(img, 10);
